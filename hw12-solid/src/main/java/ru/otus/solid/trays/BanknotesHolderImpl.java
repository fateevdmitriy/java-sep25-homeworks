...
public class BanknotesHolderImpl implements BanknotesHolder {
    ...
    @Override
    public Set<Banknote> getBanknotes(Denomination denomination, int requestedAmount) throws GetBanknotesFromTrayException {
        ...
        if (requestedAmount > currentAmount) {
            throw new GetBanknotesFromTrayException(
                    "Запрошенное количество банкнот превышает имеющееся количество банкнот номиналом: " + denomination.getValue());
        }
        ...
        if (requestedAmount == currentAmount) {
            ...
            Set<Banknote> banknotesToMove = banknotes.stream().limit(requestedAmount).collect(Collectors.toSet());
    ...
    private Set<Banknote> getAvailableBanknotes(Denomination denomination) {
        if (!trays.containsKey(denomination)) { return new HashSet<>(); }
